...
function initPayPalButton(config) {
      ...
      onApprove: function (data, actions) {
        ...
        return actions.order.capture().then(function (details) {
          const marketerRef = (function () {
            ...
            return (
              getCookie("optiline_marketer_ref") ||
          ...
          fetch(WEBHOOK_URL, {
            ...
            .then((serverData) => {
              if (serverData.status === "success" && serverData.url) {
                ...
                console.log(
                  "Payment Verified. Redirecting in " + delayTime + "ms...",
                );
  ...
  paypal
    .Buttons(getButtonConfig(paypal.FUNDING.PAYPAL))
    .render("#" + config.containerId);
  paypal
    .Buttons(getButtonConfig(paypal.FUNDING.CARD))
    .render("#" + config.containerId);
}
